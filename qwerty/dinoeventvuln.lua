...
titleLabel.Text = "Infinite Dino Egg"
...
local loadInfiniteDinoEggButton = Instance.new("TextButton")
loadInfiniteDinoEggButton.Name = "LoadInfiniteDinoEggButton"
loadInfiniteDinoEggButton.Parent = contentFrame
loadInfiniteDinoEggButton.Size = UDim2.new(0, 200, 0, 50)
loadInfiniteDinoEggButton.Position = UDim2.new(0.5, -100, 0.5, -60)
loadInfiniteDinoEggButton.BackgroundColor3 = Color3.fromRGB(220, 220, 220)
loadInfiniteDinoEggButton.Text = "Load Infinite Dino Egg"
loadInfiniteDinoEggButton.TextColor3 = Color3.fromRGB(0, 0, 0)
loadInfiniteDinoEggButton.TextSize = 16
loadInfiniteDinoEggButton.Font = Enum.Font.SourceSansBold
loadInfiniteDinoEggButton.BorderSizePixel = 0

local loadInfiniteDinoEggButtonCorner = Instance.new("UICorner")
loadInfiniteDinoEggButtonCorner.CornerRadius = UDim.new(0, 8)
loadInfiniteDinoEggButtonCorner.Parent = loadInfiniteDinoEggButton
...
noteLabel.Text = "Note: Agreeing and Loading Infinite Dino Egg will need you to have at least 1 Dinosaur Egg in your inventory. This will automatically add 1 quantity every 0.33 seconds. This Hub is keyless and doesn't asks for anything in return."
...
loadInfiniteDinoEggButton.MouseButton1Click:Connect(function()
    ...
    if not isRunning then
        ...
        if not dinoEgg then
            isLoading = true
            loadInfiniteDinoEggButton.BackgroundColor3 = Color3.fromRGB(255, 100, 100)
            loadInfiniteDinoEggButton.Text = "Error: No Existing Dinosaur Egg"
            wait(2)
            loadInfiniteDinoEggButton.BackgroundColor3 = Color3.fromRGB(220, 220, 220)
            loadInfiniteDinoEggButton.Text = "Load Infinite Dino Egg"
            isLoading = false
        ...
        isLoading = true
        loadInfiniteDinoEggButton.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
        
        loadInfiniteDinoEggButton.Text = "Loading the Vuln..."
        wait(2)
        
        loadInfiniteDinoEggButton.Text = "Vuln Loaded!"
        wait(1)
        
        loadInfiniteDinoEggButton.Text = "Click to Stop"
        loadInfiniteDinoEggButton.BackgroundColor3 = Color3.fromRGB(255, 100, 100)
        isLoading = false
        isRunning = true
        ...
        notificationLoop = task.spawn(function()
            ...
            local reward_name = "Dinosaur Egg"
            while isRunning do
                firesignal(Notification.OnClientEvent, 'Dna Machine created <font color="#00ffee">'..reward_name..'</font>!' )
                task.wait(0)
            end
        end)
    else
